...
PARSE_SYSTEM_PROMPT = (_PROMPT_DIR / "rule_parse.md").read_text(encoding="utf-8")
COMPACT_PARSE_SYSTEM_PROMPT = (_PROMPT_DIR / "rule_parse_compact.md").read_text(encoding="utf-8")
...
请输出JSON格式的规则："""
        ...
        try:
            ...
            llm = ChatOpenAI(
                ...
                temperature=0,
                timeout=300,  # 放宽超时，优先保证大文档可完成
                max_tokens=12000,  # 提高上限，避免 JSON 被截断
            )
            ...
            try:
                ...
                # 非流式失败：先走精简重试（快速），最后才兜底流式
                logger.warning(f"非流式调用失败({invoke_err})，先尝试精简重试...")
                compact_data = self._extract_rules_with_llm_compact(text_for_llm[:25000], filename)
                if isinstance(compact_data, dict):
                    data = compact_data
                    logger.info("精简重试成功，跳过流式回退")
                else:
                    logger.warning("精简重试失败，回退到流式接收...")
                    full_content = ""
                    for chunk in self._extract_rules_with_llm_stream(text, filename):
                        if isinstance(chunk, str):
                            full_content += chunk
                    if not full_content.strip():
                        raise RuntimeError("流式回退未获取到有效内容")
                    return self.parse_stream_result(full_content, filename)
            ...
            if data is None:
                ...
                # 二次重试：使用精简提示词，降低输出长度，避免截断
                data = self._extract_rules_with_llm_compact(text_for_llm, filename)
                if data is None:
                    return BrandRules(
                        brand_id="",
                        brand_name="",
...
请输出JSON格式的规则："""
    ...
    def _extract_rules_with_llm_compact(self, text: str, filename: str) -> dict | None:
        """解析失败后的精简重试，目标是返回可解析 JSON。"""
        from langchain_core.messages import HumanMessage, SystemMessage
        from langchain_openai import ChatOpenAI
        ...
        try:
            ...
            messages = [
                SystemMessage(content=COMPACT_PARSE_SYSTEM_PROMPT),
                HumanMessage(content=f"文档内容如下：\n{text}\n\n请仅返回 JSON。"),
            ]
            resp = llm.invoke(messages)
            compact_content = resp.content if hasattr(resp, 'content') else str(resp)
            data = self._parse_json_response(compact_content)
            if data is None:
                logger.error("精简重试仍无法解析JSON")
            return data if isinstance(data, dict) else None
        except Exception as e:
            logger.error(f"精简重试失败: {type(e).__name__}: {e}")
            return None
